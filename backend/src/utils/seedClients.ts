...
export const seedClients = async () => {
  try {
    ...
    const existingClientsCount = await prisma.company.count({
      where: { type: 'client' }
    });
    
    if (existingClientsCount > 0) {
      console.log(`📊 Já existem ${existingClientsCount} clientes na base. Pulando seed...`);
      return;
    }
    ...
    for (const clientData of clientsData) {
      ...
      console.log(`✅ Cliente criado: ${clientData.companyName}`);
    }
    
    console.log(`🎉 ${clientsData.length} clientes de exemplo inseridos com sucesso!`);
