...
const ViewCustomerModal = ({ open, onOpenChange, customerId }: ViewCustomerModalProps) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[900px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Complete information about the customer company
          </DialogDescription>
        </DialogHeader>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{customer.assignedTo || 'Unassigned'}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{customer.wcClass ? `Class ${customer.wcClass}` : 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Markup</p>
                  <p>
                    {customer.markupType && customer.markupValue ? (
                      customer.markupType === 'Percent' 
                        ? `${customer.markupValue}%` 
                        : formatCurrency(parseFloat(customer.markupValue))
                    ) : 'Not specified'}
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Commission</p>
                  <p>{customer.commission ? `${customer.commission}%` : 'Not specified'}</p>
                </div>
              </div>
            </div>
              ...
              {customer.bills && customer.bills.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Showing the most recent {customer.bills.length} bills
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Table>
                      ...
                      <TableBody>
                        {customer.bills.map((bill: any) => (
                          <TableRow key={bill.id}>
                            ...
                            <TableCell>{formatCurrency(parseFloat(bill.totalCustomer))}</TableCell>
                            <TableCell>
                              <Badge variant={
