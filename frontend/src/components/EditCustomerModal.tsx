...
const EditCustomerModal = ({ open, onOpenChange, customerId, onSuccess }: EditCustomerModalProps) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          wcClass: customer.wcClass || 'none',
          markupType: customer.markupType || 'none',
          markupValue: customer.markupValue ? customer.markupValue.toString() : '',
          commission: customer.commission ? customer.commission.toString() : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiService.updateCustomer(customerId!, submitData);
  ...
  if (!originalData && !loadingCustomer) {
    return null;
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {loadingCustomer ? "Loading customer information..." : `Update information for ${originalData?.companyName}`}
          </DialogDescription>
        </DialogHeader>
